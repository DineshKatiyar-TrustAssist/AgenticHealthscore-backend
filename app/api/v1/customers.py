...
async def calculate_customer_health_score(
    ...
    try:
        ...
        result = await orchestrator.analyze_customer(
        ...
        if result.get("status") == "success":
            return HealthScoreCalculateResponse(
                ...
                health_score=HealthScoreResponse(
                    id=UUID(int=0),  # Placeholder - would need to fetch actual ID
                    customer_id=customer_id,
        ...
        return HealthScoreCalculateResponse(
            status="error",
            customer_id=customer_id,
            messages_fetched=total_messages_fetched if 'total_messages_fetched' in locals() else 0,
            error=str(e),
        )
